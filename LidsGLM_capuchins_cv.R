...
null1=glmer(any_peek~ 1 + (1+Condition + z.Trialnumb| ID), family = binomial, control=contr)
...
null1b=glmer(peek_below~ 1 + (1+Condition + z.Trialnumb| ID), family = binomial, control=contr)
...
null1c=glmer(peek_above~ 1 + (1+Condition + z.Trialnumb| ID), family = binomial, control=contr)
...
GLM2a=glmer(peek_above ~ Configuration #(all baited/all open/ mixed)
            + z.Trialnumb + (1+Configuration + z.Trialnumb |ID), family =binomial, control=contr)

null2a= glmer(peek_above ~ 1 + (1+Configuration + z.Trialnumb |ID), family =binomial, control=contr)
...
GLM2b=glmer(peek_below ~ Configuration + z.Trialnumb +(1+Configuration + z.Trialnumb |ID), family =binomial, control=contr)

null2b= glmer(peek_below ~ 1 + (1+Configuration + z.Trialnumb |ID), family =binomial, control=contr)
...
GLM2c=glmer(any_peek ~ Configuration + z.Trialnumb +  (1+Configuration + z.Trialnumb |ID), family =binomial, control=contr)

null2c= glmer(any_peek ~ 1 +  (1+Configuration + z.Trialnumb |ID), family =binomial, control=contr)
...
null3=glmer(correct~ 1 + (1|ID) + (0+Condition*Configuration |ID)+ (0+z.Trialnumb|ID), family = binomial, control=contr)
...
#subsetting data following significant interaction
###
...
GLMM3a=glmer(correct~Configuration + z.Trialnumb + (1|ID) + (0+Configuration |ID)+ (0+z.Trialnumb|ID), family = binomial, control=contr)

null3a=glmer(correct~1 + (1|ID) + (0+Configuration |ID)+ (0+z.Trialnumb|ID), family = binomial, control=contr)
...
transparent=subset(Lidsdata,Condition=="Transparent")

detach(occlude)
attach(transparent)
z.Trialnumb=as.vector(scale(trial_number))##CV:after subsetting the data you need to do the scaling again.

GLMM3b=glmer(correct~Configuration + z.Trialnumb + (1|ID) + (0+Configuration |ID)+ (0+z.Trialnumb|ID), family = binomial, control=contr)

null3b=glmer(correct~1 + (1|ID) + (0+Configuration |ID)+ (0+z.Trialnumb|ID), family = binomial, control=contr)
...
#post-hoc tests
xx=glht(GLMM3b, linfct = mcp(Configuration = c("All_Baited-Mixed = 0", "All_Baited-All_Open = 0", "Mixed-All_Open = 0")))
summary(xx)
